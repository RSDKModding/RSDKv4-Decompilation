...
inline uint64_t GETBIT(uint64_t i, int n) {return (i & (1 << n)) >> n;};
...
string generateCode(ushort port, int gameLength, int itemMode) {
    ...
    int i = 0;
    while (code >= (uint64_t)pow(36, i + 1)) 
        i++;
...
int getIP(uint* ip) {
    ...
    try {
        ...
        if (status_code != 200) return status_code; //not OK
        //read the header
        std::string header;
