...
#define XDG_DATA_PATH "/RSDKv4"
#define XDG_FULL_PATH "/.local/share" XDG_DATA_PATH

char const *getXDGDataPath()
{
    char *dataHome = getenv("XDG_DATA_HOME");
    if (dataHome == NULL || dataHome[0] == '\0') {
        char *home      = getenv("HOME");
        size_t homeSize = strlen(home);
        size_t pathSize = homeSize + sizeof XDG_FULL_PATH + 1;
        char *path      = (char *)malloc(pathSize);

        memcpy(path, home, homeSize);
        memcpy(path + homeSize, XDG_FULL_PATH, sizeof XDG_FULL_PATH);

        return path;
    }
    else {
        size_t dataHomeSize = strlen(dataHome);
        size_t pathSize     = dataHomeSize + sizeof XDG_DATA_PATH + 1;
        char *path          = (char *)malloc(pathSize);

        memcpy(path, dataHome, dataHomeSize);
        memcpy(path + dataHomeSize, XDG_DATA_PATH, sizeof XDG_DATA_PATH);
        free(dataHome);
...
bool ReadSaveRAMData()
{
    ...
    sprintf(buffer, "%s/%sSData.bin", redirectSave ? modsPath : getXDGDataPath(), savePath);
#else
    sprintf(buffer, "%s%sSData.bin", redirectSave ? modsPath : gamePath, savePath);
#endif
...
#elif RETRO_PLATFORM == RETRO_LINUX
        sprintf(buffer, "%s/%sSGame.bin", redirectSave ? modsPath : getXDGDataPath(), savePath);
#else
        sprintf(buffer, "%s%sSGame.bin", redirectSave ? modsPath : gamePath, savePath);
#endif
...
void InitUserdata()
{
    ...
    mkdir(gamePath, 0777);
#elif RETRO_PLATFORM == RETRO_LINUX
    sprintf(gamePath, "%s", getXDGDataPath());
    sprintf(modsPath, "%s/", getXDGDataPath());

    mkdir(getXDGDataPath(), 0755);
...
#elif RETRO_PLATFORM == RETRO_LINUX
    sprintf(buffer, "%s/settings.ini", getXDGDataPath());
#else
...
#elif RETRO_PLATFORM == RETRO_LINUX
    sprintf(buffer, "%s/UData.bin", getXDGDataPath());
#else
    sprintf(buffer, "%sUData.bin", gamePath);
#endif
    file = fOpen(buffer, "rb");
...
#elif RETRO_PLATFORM == RETRO_LINUX
    sprintf(buffer, "%s/%sUData.bin", redirectSave ? modsPath : getXDGDataPath(), savePath);
#else
    sprintf(buffer, "%s%sUData.bin", redirectSave ? modsPath : gamePath, savePath);
#endif
