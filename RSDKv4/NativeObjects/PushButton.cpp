...
void PushButton_Main(void *objPtr)
{
    ...
    switch (self->state) {
        case PUSHBUTTON_STATE_UNSELECTED: {
            ...
            RenderImage(self->x - self->textWidth, self->y, self->z, self->scale, self->scale, 64.0, 64.0, 64.0, 128.0, 0.0, 0.0,
                        self->alpha, self->symbolsTex);
            RenderImage(self->x, self->y, self->z, self->textWidth + self->textWidth, self->scale, 0.5, 64.0, 1.0, 128.0, 63.0, 0.0,
                        self->alpha, self->symbolsTex);
            RenderImage(self->x + self->textWidth, self->y, self->z, self->scale, self->scale, 0.0, 64.0, 64.0, 128.0, 64.0, 0.0,
                        self->alpha, self->symbolsTex);
        ...
        case PUSHBUTTON_STATE_SELECTED: {
            if (usePhysicalControls) {
                ...
                RenderImage(self->x - self->textWidth, self->y, self->z, 1.1 * self->scale, 1.1 * self->scale, 64.0, 64.0, 64.0, 128.0,
                            0.0, 0.0, self->alpha, self->symbolsTex);
                RenderImage(self->x, self->y, self->z, self->textWidth + self->textWidth, 1.1 * self->scale, 0.5, 64.0, 1.0, 128.0, 63.0,
                            0.0, self->alpha, self->symbolsTex);
                RenderImage(self->x + self->textWidth, self->y, self->z, 1.1 * self->scale, 1.1 * self->scale, 0.0, 64.0, 64.0, 128.0,
                            64.0, 0.0, self->alpha, self->symbolsTex);
            }
