This branch of the RSDKv4 Decompilation targets running the version of Sonic CD included in the Sega Classics (Amazon Fire TV) compilation. It is not intended to run other games; those should work on the main branch as is.

It was decided to isolate support for Sega Classics Sonic CD as a separate branch, as including it in master would make the codebase messy, which isn't worth it for supporting an obscure release.

# Differences from main branch

- Added the OGV video functionality from RSDKv3 (Videos are called by running `CallNativeFunction2(PlayVideo, "VideoFileNameWithoutExtension", 0)` in a script, otherwise works the same as the v3 decomp)
- Modified the main menu to match Sega Classics Sonic CD's menu (Note that this is a functional recreation of the menu rather than a direct decomp of it)
- Use 3DFloorLayer instead of 3DSkyLayer to fix an issue with CD special stages
- Other misc fixes

# I found a bug!

While we do accept bug reports in the Issues tab, keep in mind that the Sega Classics version of Sonic CD is a port that was literally left unfinished. There's a good chance the bug you're encountering is within the game itself and not caused by the decompilation.
...
`git clone --recursive https://github.com/RSDKModding/RSDKv4-Decompilation --branch sega-classics`
